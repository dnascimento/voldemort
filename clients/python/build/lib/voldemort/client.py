...
class StoreClient:
    ...
    def _get_with_connection(self, connection, store_name, key, should_route,rid = None):
    ...
    def get(self, key, rid = None):
        ...
        return [(self.value_serializer.reads(value), version)
                for value, version in self._execute_request(self._get, [raw_key, rid])]
    ...
    def _put(self, key, value, version,rid = None):
        req = protocol.VoldemortRequest()
        req.should_route = True
        req.store = self.store_name
    ...
    def put(self, key, value, version = None,rid = None):
        ...
        return self._execute_request(self._put, [raw_key, raw_value, version,rid])

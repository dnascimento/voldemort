...
public class DefaultStoreClientTest {
    ...
    @Test
    public void testGet() {
        assertEquals("GET of non-existant key should return null.", null, client.get("k", 0L));
        client.put("k", "v", 0L);
        assertEquals("After a PUT get should return the value", "v", client.get("k", 0L).getValue());
        assertNotNull("The version of the value found should be non-null", client.get("k", 0L)
                                                                                 .getVersion());
    }
    ...
    public void testGetWithDefault() {
        ...
        assertEquals("If there is a value for k, get(k) should return it.",
                     ...
                     client.get("k", new Versioned<String>("v2"), 0L));
        assertNotNull(client.get("k", 0L).getVersion());
    }

    @Test
    public void testGetUnversioned() {
        assertEquals("GET of non-existant key should be null.", null, client.getValue("k", 0L));
        client.put("k", "v", 0L);
        assertEquals("GET of k should return v, if v is there.", "v", client.getValue("k", 0L));
    }
    ...
    public void testGetUnversionedWithDefault() {
        assertEquals("GET of non-existant key should return default.",
                     "v",
                     client.getValue("k", "v", 0L));
        assertEquals("null should be an acceptable default", null, client.getValue("k", null, 0L));
        client.put("k", "v", 0L);
        assertEquals("default should not be returned if value is present.",
                     "v",
                     client.getValue("k", "v2", 0L));
    }

    @Test
    public void testPutVersioned() {
        client.put("k", Versioned.value("v"), 0L);
    ...
    public void testPutUnversioned() {
        client.put("k", "v", 0l);
        assertEquals("GET should fetch the value set by PUT", "v", client.getValue("k", 0L));
        client.put("k", "v2", 0l);
        assertEquals("Overwrite of value should succeed.", "v2", client.getValue("k", 0l));
    }

    @Test
    public void testPutIfNotObsolete() {
        client.putIfNotObsolete("k", new Versioned<String>("v"), 0l);
        assertEquals("PUT of non-obsolete version should succeed.", "v", client.getValue("k", 0l));
        assertFalse(client.putIfNotObsolete("k", new Versioned<String>("v2"), 0L));
    ...
    public void testDelete() {
        assertFalse("Delete of non-existant key should be false.", client.delete("k", 0l));
        client.put("k", "v", 0L);
        assertTrue("Delete of contained key should be true", client.delete("k", 0l));
    ...
    public void testDeleteVersion() {
        assertFalse("Delete of non-existant key should be false.",
                    client.delete("k", new VectorClock(), 0L));
        client.put("k", new Versioned<String>("v"), 0L);
    ...
    public void testGetAll() {
        ...
        Map<String, Versioned<String>> result = client.getAll(Arrays.asList("k", "l"), 0L);
        assertEquals(2, result.size());
        assertEquals("v", result.get("k").getValue());
        assertEquals("m", result.get("l").getValue());

        result = client.getAll(Arrays.asList("m", "s"), 0L);
        assertNotNull(client.get("k", 0L).getVersion());

...
public class DataCleanupJobTest {
    ...
    public void testCleanupFrequency() {
        ...
        try {
            ...
            for(int i = 0; i < 10; i++) {
                ...
                engine.put(b, new Versioned<byte[]>(b.get()), null, new SRD());
            }
            // sleep for 2 seconds
            ...
            for(int i = 0; i < 10; i++) {
                ByteArray b = new ByteArray(Integer.toString(i).getBytes());
                List<Versioned<byte[]>> found = engine.get(b, null, new SRD());
    ...
    private void runRetentionEnforcingStoreTest(boolean onlineDeletes) throws InterruptedException {
        ...
        assertEquals("k1 should have expired",
                     0,
                     store.get(new ByteArray("k1".getBytes()), null, new SRD()).size());
        assertEquals("k2 should have expired",
                     0,
                     store.get(new ByteArray("k2".getBytes()), null, new SRD()).size());

        assertTrue("k3 should not have expired",
                   store.get(new ByteArray("k3".getBytes()), null, new SRD()).size() > 0);
        assertTrue("k4 should not have expired",
                   store.get(new ByteArray("k4".getBytes()), null, new SRD()).size() > 0);
        ...
        assertEquals("k1 should be present",
                     !onlineDeletes,
                     engine.get(new ByteArray("k1".getBytes()), null, new SRD()).size() > 0);
        assertEquals("k2 should be present",
                     !onlineDeletes,
                     engine.get(new ByteArray("k2".getBytes()), null, new SRD()).size() > 0);

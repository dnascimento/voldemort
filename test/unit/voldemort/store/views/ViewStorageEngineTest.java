...
public class ViewStorageEngineTest extends TestCase {
    ...
    @Override
    public void setUp() {
        target1.put("hello", Versioned.value("world"), null, 0L);
        Integer[] values1 = { 1, 2, 3, 4, 5, 6, 7, 8 };
        Integer[] values2 = { 100, 200, 300, 400, 500, 600, 700 };
        target2.put(1, Versioned.value(Arrays.asList(values1)), null, 0L);
        target2.put(100, Versioned.value(Arrays.asList(values2)), null, 0L);
    }
    ...
    public void testGetWithValueTransform() {
        assertEquals("View should add 42", "world42", valView.get("hello", "concat", 0L)
                                                             .get(0)
                                                             .getValue());
        assertEquals("Null value should return empty list", 0, valView.get("laksjdf", "concat", 0L)
                                                                      .size());
    }

    public void testGetAll() {
        target1.put("a", Versioned.value("a"), null, 0L);
        target1.put("b", Versioned.value("b"), null, 0L);
    ...
    public void testPut() {
        valView.put("abc", Versioned.value("cde"), null, 0L);
        assertEquals("c", target1.get("abc", null, 0L).get(0).getValue());
    }

    public void testGetWithTransforms() {
        Integer[] filter2 = { 5, 8 };
        assertEquals(4, view.get(1, Arrays.asList(filter2), 0L).get(0).getValue().size());

        Integer[] filter1 = { 1, 5 };
        assertEquals(5, view.get(1, Arrays.asList(filter1), 0L).get(0).getValue().size());
    ...
    public void testPutWithTransforms() {
        ...
        view.put(1, Versioned.value(values.getValue(), clock), Arrays.asList(filter1), 0L);

        assertEquals(10, view.get(1, Arrays.asList(filter1), 0L).get(0).getValue().size());

        Integer[] filter2 = { 5, 10 };
        assertEquals(6, view.get(1, Arrays.asList(filter2), 0L).get(0).getValue().size());

        Version updatedVersion = view.get(1, Arrays.asList(filter2), 0L).get(0).getVersion();
        ...
        view.put(1, Versioned.value(Arrays.asList(values2), clock1), Arrays.asList(filter3), 0L);

        assertEquals(12, view.get(1, Arrays.asList(filter3), 0L).get(0).getValue().size());
    }

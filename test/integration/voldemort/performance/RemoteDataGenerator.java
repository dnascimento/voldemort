...
public class RemoteDataGenerator {
    ...
    public void generateData(int requests, int keySize, int valueSize, String postfix) {
        ...
        for(int i = 0; i < requests; i++) {
            ...
            try {
                client.put(keyBuilder.toString(), valueBuilder.toString(), new SRD());
            } catch(Exception e) {
                e.printStackTrace();
            }
        }
    ...
    /**
     * Pads a string with random longs until it reaches
     * 
     * @param size bytes
     * @param size Desired bytes in the string
     * @return String of length >=
     * @param size composed of pseudo-random longs
     */

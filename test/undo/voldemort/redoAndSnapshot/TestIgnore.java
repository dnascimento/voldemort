...
public class TestIgnore {
    ...
    class DoOp extends Thread {
        ...
        public void run() {
            order.add("Try read");
            entry.redoRead(OpType.Get, new SRD(22, 1, false), path);
            order.add("readed");
        }
    }
    ...
    public void testIgnore() throws InterruptedException {
        ...
        entry = new OpMultimapEntry(key);
        StsBranchPair sts = new StsBranchPair(0, 0);
        HashSet<StsBranchPair> branch = new HashSet<StsBranchPair>();
        branch.add(sts);
        path = new BranchPath(sts, branch);

        entry.getVersionToPut(new SRD(21, 1, false), path);
        entry.trackWriteAccess(OpType.Put, new SRD(21, 1, false), path);
        entry.trackReadAccess(new SRD(22, 1, false), path);
